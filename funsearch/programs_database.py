...
print(f"programs_database: main - Python version: {'.'.join(map(str, sys.version_info[:3]))}")
...
class ProgramsDatabase:
  ...
  def _register_program_in_island(
    ...
    if score > self._best_score_per_island[island_id]:
      now = time.time()
      elapsed_time = (now - self._last_best_score_time) / 60  # Convert to minutes
      self._last_best_score_time = now  # Update the timestamp
